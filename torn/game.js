...
function initState() {
  ...
  // If saved daily exists but pool has grown, force re-pick
  if (existing && existing.images && existing.images.length > 0) {
    if (existing.poolSize && existing.poolSize < pool.length) {
      // Pool grew, pick new images
    } else {
      state = existing;
      if (state.finished) {
  ...
  // New day, first time, or pool grew — pick fresh images
