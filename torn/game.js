...
function getPool() {
  const defaultPool = (typeof DEFAULT_POOL !== 'undefined' && Array.isArray(DEFAULT_POOL) && DEFAULT_POOL.length > 0) ? DEFAULT_POOL : [];
  ...
  if (defaultPool.length >= local.length && defaultPool.length > 0) {
    return defaultPool;
  }
...
function initState() {
  ...
  // Validate existing daily state images
  const existingValid = existing && Array.isArray(existing.images) &&
    existing.images.length === targetCount &&
    existing.images.every(img => img && typeof img.url === 'string' && img.url.length > 0);

  if (existingValid && !existing.finished) {
    state = existing;
    return pool.length;
  }
...
function render() {
  ...
  if (current && current.url) {
    ...
    img.src = 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="400" height="400"><rect fill="%231a1a24" width="400" height="400"/><text fill="%23c084fc" x="50%" y="45%" text-anchor="middle" font-family="sans-serif" font-weight="bold" font-size="20">No Image Data</text><text fill="%23888" x="50%" y="58%" text-anchor="middle" font-family="sans-serif" font-size="14">Re-upload images in Admin Panel</text></svg>';
  }
  img.onerror = function() {
    img.src = 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="400" height="400"><rect fill="%231a1a24" width="400" height="400"/><text fill="%23ef4444" x="50%" y="45%" text-anchor="middle" font-family="sans-serif" font-weight="bold" font-size="20">Image Failed to Load</text><text fill="%23888" x="50%" y="58%" text-anchor="middle" font-family="sans-serif" font-size="14">Re-upload images in Admin Panel</text></svg>';
  };
  ...
  frame.setAttribute('data-tooltip', 'Gender: ' + (current ? (current.gender === 'M' ? 'Male' : 'Female') : 'Unknown') + ' — Is this person transgender?');
  ...
  submitBtn.textContent = 'Select Transgender or Not Trans';
}
...
function submitVote() {
  ...
  const current = state.images[state.currentIndex];
  const isTrans = current ? (current.trans === true) : false;
