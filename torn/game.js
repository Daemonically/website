...
function initState() {
  ...
  // New day or first time — pick fresh images from pool (does NOT delete them)
...
function restartDay() {
  console.log('restartDay called, current state:', state);
  state.currentIndex = 0;
  state.score = 0;
  state.votes = [];
  state.finished = false;
  localStorage.setItem(STORAGE_KEY_DAY, JSON.stringify(state));
  console.log('restartDay saved, calling render');
