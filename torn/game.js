// game.js — Core game engine with effects
...
// --- Data helpers ---
function getPool() {
  try { return JSON.parse(localStorage.getItem(STORAGE_KEY_IMG)) || []; }
  catch { return []; }
}

function savePool(pool) {
  localStorage.setItem(STORAGE_KEY_IMG, JSON.stringify(pool));
}
...
// --- Daily selection + delete from pool ---
function pickDailyImages(pool) {
  if (pool.length === 0) return { chosen: [], remaining: pool };
...
function initState() {
  ...
  if (existing && existing.images.length > 0) {
    state = existing;
    // Restore remaining pool from state if available
    dailyPool = existing.remainingPool || [];
    return pool.length;
  }

  const { chosen, remaining } = pickDailyImages(pool);
  dailyPool = remaining;
...
function render() {
  ...
  if (state.finished || state.currentIndex >= state.images.length) {
    document.querySelector('.image-frame').style.display = 'none';
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (state.images.length < TOTAL_ROUNDS) {
    document.getElementById('max-score').textContent = state.images.length;
  }
